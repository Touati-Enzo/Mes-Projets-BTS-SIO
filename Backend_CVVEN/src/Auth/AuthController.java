...
public class AuthController implements Initializable {
	...
	private void Authentification(ActionEvent event) {
	    ...
	    if (conn == null) {
	        JOptionPane.showMessageDialog(null, "Connexion à la base impossible");
	        return;
	    }

	    try {
	        PreparedStatement st = conn.prepareStatement(
	            "SELECT login, mdp FROM gestion_auth WHERE login=? AND mdp=?"
	        );
	        ...
	        if (rs.next()) {
	            ...
	            stage.setScene(new Scene(FXMLLoader.load(getClass().getResource("/Dashboard/Dashboard.fxml"))));
